...
__global__ void mover_PC_gpu(particles* part_gpu, EMfield* field_gpu, grid* grid_gpu, parameters* param_gpu)
{
    ...
    // FPpart qom;
    newDT qomGPU = __float2bfloat16(part_gpu->qom);

    // // FPpart u0, v0, w0;
    // newDT u0GPU = __float2bfloat16(part_gpu->u0)
    // newDT v0GPU = __float2bfloat16(part_gpu->v0)
    // newDT w0GPU = __float2bfloat16(part_gpu->w0)
    // FPpart uth, vth, wth;
    // newDT uthGPU = __float2bfloat16(part_gpu->uth)
    // newDT vthGPU = __float2bfloat16(part_gpu->vth)
    // newDT wthGPU = __float2bfloat16(part_gpu->wth)

    newDT x_idx = __float2bfloat16(part_gpu->x[idx]);
    newDT y_idx = __float2bfloat16(part_gpu->y[idx]);
    newDT z_idx = __float2bfloat16(part_gpu->z[idx]);
    newDT u_idx = __float2bfloat16(part_gpu->u[idx]);
    newDT v_idx = __float2bfloat16(part_gpu->v[idx]);
    newDT w_idx = __float2bfloat16(part_gpu->w[idx]);
    ...
    newDT dt_sub_cycling = __double2bfloat16(param_gpu->dt)/ __float2bfloat16(part_gpu->n_sub_cycles);

    newDT dto2 = __float2bfloat16(0.5) * dt_sub_cycling;
    newDT qomdt2 = qomGPU*dto2/__double2bfloat16(param_gpu->c);
    ...
    for (int i_sub=0; i_sub <  part_gpu->n_sub_cycles; i_sub++){
        ...
        // for (int idx=idx; idx <  part_gpu->nop; idx+=stride){
            ...
            for(int innter=0; innter < part_gpu->NiterMover; innter++){
                ...
                ix = 2 +  int((x_idx - __float2bfloat16((grid_gpu->xStart)*grid_gpu->invdx)));
                iy = 2 +  int((y_idx - __float2bfloat16((grid_gpu->yStart)*grid_gpu->invdy)));
                iz = 2 +  int((z_idx - __float2bfloat16((grid_gpu->zStart)*grid_gpu->invdz)));
                ...
                xi[0]   = x_idx - __float2bfloat16(grid_gpu->XN_flat[get_idx(ix - 1, iy, iz, grid_gpu->nyn, grid_gpu->nzn)]);
                eta[0]  = y_idx - __float2bfloat16(grid_gpu->YN_flat[get_idx(ix, iy - 1, iz, grid_gpu->nyn, grid_gpu->nzn)]);
                zeta[0] = z_idx - __float2bfloat16(grid_gpu->ZN_flat[get_idx(ix, iy, iz - 1, grid_gpu->nyn, grid_gpu->nzn)]);
                xi[1]   = __float2bfloat16(grid_gpu->XN_flat[get_idx(ix, iy, iz, grid_gpu->nyn, grid_gpu->nzn)]) - x_idx;
                eta[1]  = __float2bfloat16(grid_gpu->YN_flat[get_idx(ix, iy, iz, grid_gpu->nyn, grid_gpu->nzn)]) - y_idx;
                zeta[1] = __float2bfloat16(grid_gpu->ZN_flat[get_idx(ix, iy, iz, grid_gpu->nyn, grid_gpu->nzn)]) - z_idx;

                for (int ii = 0; ii < 2; ii++)
                    for (int jj = 0; jj < 2; jj++)
                        for (int kk = 0; kk < 2; kk++)
                            weight[ii][jj][kk] = __float2bfloat16(xi[ii]) * __float2bfloat16(eta[jj]) * __float2bfloat16(zeta[kk]) * __float2bfloat16(grid_gpu->invVOL);
                ...
                ///////////////////////////////////////////
                            ...
                            Exl = Exl + weight[ii][jj][kk]*__float2bfloat16(field_gpu->Ex_flat[get_idx(ix-ii, iy-jj, iz-kk, grid_gpu->nyn, grid_gpu->nzn)]);
                            Eyl = Eyl + weight[ii][jj][kk]*__float2bfloat16(field_gpu->Ey_flat[get_idx(ix-ii, iy-jj, iz-kk, grid_gpu->nyn, grid_gpu->nzn)]);
                            Ezl = Ezl + weight[ii][jj][kk]*__float2bfloat16(field_gpu->Ez_flat[get_idx(ix-ii, iy-jj, iz-kk, grid_gpu->nyn, grid_gpu->nzn)]);
                            Bxl = Bxl + weight[ii][jj][kk]*__float2bfloat16(field_gpu->Bxn_flat[get_idx(ix-ii, iy-jj, iz-kk, grid_gpu->nyn, grid_gpu->nzn)]);
                            Byl = Byl + weight[ii][jj][kk]*__float2bfloat16(field_gpu->Byn_flat[get_idx(ix-ii, iy-jj, iz-kk, grid_gpu->nyn, grid_gpu->nzn)]);
                            Bzl = Bzl + weight[ii][jj][kk]*__float2bfloat16(field_gpu->Bzn_flat[get_idx(ix-ii, iy-jj, iz-kk, grid_gpu->nyn, grid_gpu->nzn)]);
                        }

                // end interpolation
                omdtsq = qomdt2*qomdt2*(Bxl*Bxl+Byl*Byl+Bzl*Bzl);
                denom = __float2bfloat16(1.0) / ( __float2bfloat16(1.0) + omdtsq);
            ...
            u_idx= __float2bfloat16(2.0) * uptilde - u_idx;
            v_idx= __float2bfloat16(2.0) * vptilde - v_idx;
            w_idx= __float2bfloat16(2.0) * wptilde - w_idx;
            ...
            part_gpu->x[idx] = __bfloat162float(x_idx);
            part_gpu->y[idx] = __bfloat162float(y_idx);
            part_gpu->z[idx] = __bfloat162float(z_idx);
            part_gpu->u[idx] = __bfloat162float(u_idx);
            part_gpu->v[idx] = __bfloat162float(v_idx);
            part_gpu->w[idx] = __bfloat162float(w_idx);
